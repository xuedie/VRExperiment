...
public class FittsDragActivity extends FittsActivity implements View.OnTouchListener
{
    ...
    public void doFingerDown(float xArg, float yArg)
    {
        ...
        boolean isTarget = expPanel.toTarget.inTarget(xArg, yArg);
        if (isTarget) {
            expPanel.isDragging = true;
            fingerDownMiss = 0;
            expPanel.draggingTarget.copyTarget(expPanel.toTarget);
            draggingTarget(xArg, yArg);
            Log.d("DOWN", String.format("%.02f", xArg) + " " + String.format("%.02f", yArg));
        } else
        {
            fingerDownMiss = 1;
        }
    ...
    {
        boolean isTarget = expPanel.toTarget.inTarget(xArg, yArg);
        boolean isDestTarget = expPanel.destTarget.circleInTarget(xArg, yArg);
        expPanel.destTarget.status = Target.NORMAL;
        if (isTarget && !expPanel.isDragging) {
            expPanel.isDragging = true;
            expPanel.draggingTarget.copyTarget(expPanel.toTarget);
        }
        if (expPanel.isDragging)
        {
            draggingTarget(xArg, yArg);
            Log.d("MOVE", "dragging");
            if (isDestTarget) {
                expPanel.destTarget.status = Target.DRAGGINGSELECTED;
            }
        }
        Log.d("MOVE", String.format("%.02f", xArg) + " " + String.format("%.02f", yArg));
    }

    void draggingTarget(float xArg, float yArg)
    {
        expPanel.draggingTarget.moveTarget(xArg, yArg);
    }

    public void doStartCircleSelected()
    {
        super.doStartCircleSelected();

        // set initial destination target
        expPanel.destTarget = expPanel.targetSet[5];
    }
    ...
    void doTargetSelected(float xSelect, float ySelect)
    {
        ...
        if (expPanel.isDragging)
        {
            Log.d("UP", String.format("%.02f", xSelect) + " " + String.format("%.02f", ySelect));
        }
        ...
        trialMiss = expPanel.isDragging && expPanel.destTarget.circleInTarget(xSelect, ySelect) ? 0 : 1;
        ...
        // do not let the sequence start unless the first tap is on the start target
        // if (trialMiss == 1 && !sequenceStarted)
        //    return;
    ...
    private void advanceTarget()
    {
        ...
        // set drag destination MOCK for now
        expPanel.destTarget = expPanel.targetSet[(next+5) % 12];
